...
//!   made and printed (`invite add <label> --for <key>`), so the token carries no secret and is safe over
//!   any channel. Adopting it keeps the machine's identity and stores the badge.
//! - **Derived**: `invite:<seed>.<signet>.<badge>`. The owner derived a child identity and hands over its
//!   seed (`invite add <label>`, today's `mint`): adopting it BECOMES that identity. The seed is a device
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Derived {
                ...
                signet,
                badge: None,
            } => write!(f, "{LEGACY_SCHEME}{}{SEPARATOR}{signet}", encode_seed(seed)),
        }
    }
}

/// Encode a device seed as lowercase unpadded base32.
