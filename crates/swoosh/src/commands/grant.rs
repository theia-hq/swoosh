...
//!
//! A local group: unlike the `tunnel` group, no leaf binds a transport or dials. `share` signs with this
//! node's persisted identity (the key an exposed service roots at); `attenuate` and `revoke` are wholly
