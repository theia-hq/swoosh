...
//! local verb: no transport, no reach. The identity lands in swoosh's own store (the node home, the
//! default `~/.config/swoosh/` or `--home`/`SWOOSH_HOME`) -- the SAME store `swoosh serve` binds under --
//! so the served node id matches the contact `mint` recorded. The signet lands beside it in the same
//! home, where the serve gate reads it.
//!
...
impl AdoptCmd {
    ...
    pub async fn run(self, home: &Home) -> eyre::Result<()> {
        ...
        // wipe our copy.
        crate::identity::write(&seed, home).await?;
        seed.zeroize();
        // Trust the signet: the default gate admits its devices (members) and delegates (slips). The signet
        // lands beside the identity just written, in the SAME home, which `swoosh serve` reads via
        // `crate::config::load_signet`. One command, one home.
        crate::config::write_signet(home, signet).await?;
        ...
        if let Some(badge) = &badge {
            crate::config::write_badge(home, badge).await?;
        }
