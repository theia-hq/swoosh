...
//! so an `ssh=sshd:` service presents the host key a client pins and a `swoosh share` link roots at the
...
/// points at `swoosh share` for minting a link, never at `tightbeam`.
