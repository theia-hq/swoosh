...
//! can drive the SAME pieces the product path does (notably the shared `commands::tunnel::expose::registry`
//! the `gated_diag` proof builds its exposer from). The binary (`main.rs`) owns only the CLI surface (the
