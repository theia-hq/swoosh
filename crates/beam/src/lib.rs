//! beam: receive a pushed file behind a keyed node's `beam:` service.
//!
...
//! gated. A caller that owns composition (swoosh) wraps [`receive_file`] in a gated handler and injects it
//! into tightbeam's registry, so every pushed file rides the same family gate as every other service; the
//! sender side (dial, expand directories, pipeline concurrent streams) is a swoosh client verb driving
