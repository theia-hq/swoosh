...
use clap::Args;
use swoosh::grants::{self, GrantRecord, GrantTarget, Grants};
...
mod tests {
    ...
    use swoosh::grants::{Delegation, GrantKind};
    ...
    /// admit path. This test fails the moment `serve` (the CLI half or the library's node engine) or the
    /// bin root names [`Grants`]. Rerun by hand with: `rg -n Grants src/bin/swoosh/commands/serve.rs
    /// src/bin/swoosh/main.rs src/serve.rs` from the repo root and confirm no hits; the same holds for
    /// the nauthy gate and tightbeam tunnel sources, which live in sibling repos and cannot be included
    /// here.
    #[test]
