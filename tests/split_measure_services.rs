...
async fn proof_ping_only() {
    ...
    // The family route binds the OWNER engine (owner limits: no run interval), so the member above may
    // open this second run immediately and reach the served-method check under test.
...
async fn expose(entries: &[String]) -> NodeId {
    ...
    tokio::task::spawn_local(async move {
        ...
        for entry in &entries {
            router = swoosh::serve::bind_entry(router, entry, HOST_SEED, &[]).unwrap();
        }
        router
            .expose()
            .unwrap()
