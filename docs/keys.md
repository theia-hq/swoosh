...
A bound invite carries no secret, so it is safe in transit, but it is not authenticated: the token is not
signed by the signet it names, so anyone who can swap the message can name their own key. Before the new
machine serves, compare the full signet the two sides printed, out of band. `adopt` also checks that the
...
signet, which they read with `swoosh status --key` on their own machine and send you.
